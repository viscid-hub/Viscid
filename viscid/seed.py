...
class Spline(SeedGen):
    ...
    def to_3d(self, pts_local, **kwargs):
        ...
        unew = np.linspace(0, 1, self.n+1, endpoint=True)
        x,y,z = interpolate.splev(unew,tck)
        x = x[(pts_local*self.n).astype(int)].astype(self.dtype)
        y = y[(pts_local*self.n).astype(int)].astype(self.dtype)
        z = z[(pts_local*self.n).astype(int)].astype(self.dtype)
        return np.vstack([x, y, z])
