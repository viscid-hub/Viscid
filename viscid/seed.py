...
__all__ = ['make_rotation_matrix', 'to_seeds', 'SeedGen', 'Point', 'Line',
...
class Plane(SeedGen):
    ...
    def to_3d(self, pts_local):
        ...
        return self.p0.reshape((3, 1)) + np.dot(lmn_to_xyz, pts_local)

    def to_local(self, pts_3d):
        xyz_to_lmn = self.get_rotation().T
        return np.dot(xyz_to_lmn, pts_3d - self.p0.reshape((3, 1)))
...
class Sphere(SeedGen):
    ...
    def to_3d(self, pts_local):
        ...
        a = np.empty((3, self.nr_points), dtype=self.dtype)
        a[0, :] = (r * np.sin(T) * np.cos(P))
        a[1, :] = (r * np.sin(T) * np.sin(P))
        a[2, :] = (r * np.cos(T) + 0.0 * P)

        return self.p0.reshape((3, 1)) + np.dot(self.get_rotation(), a)
    ...
    def _make_local_axes(self):
        theta = np.linspace(0, np.pi, self.ntheta + 1,
