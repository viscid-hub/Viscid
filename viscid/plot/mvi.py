...
def data_source(fld):
    ...
    for i in range(3):
        bdata[:,i] = np.asarray(collection[0]['b'][:,:,:,i], dtype='f').ravel()
...
def mlab_earth(pipeline, daycol=(1,1,1), nightcol=(0,0,0), res=15):
    ...
    night.data_source.set(center=(0,0,0), radius=1.0, start_theta=270, end_theta=90,
            theta_resolution=res, phi_resolution=res)
    pipeline.surface(night, color=nightcol)

    day = BuiltinSurface(source='sphere', name='day')
    day.data_source.set(center=(0,0,0), radius=1.0, start_theta=90, end_theta=270,
            theta_resolution=res, phi_resolution=res)
