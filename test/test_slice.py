...
def test_slice(selection, fld, dat_shape, nc_shape=None, cc_shape=None,
    ...
    assert not nc_shape is not None or all(slced_fld.crds.shape_nc == nc_shape)
    assert not cc_shape is not None or all(slced_fld.crds.shape_cc == cc_shape)
...
def main():
    ...
    test_slice(selection, fld, (1, 30, 40, 1), nc_shape=(2, 31, 41, 2), cc_shape=(1, 30, 40, 1))
    ...
    #### SLICE 3
    selection = np.s_[None, ..., None, 1]
    test_slice(selection, fld, (1, 31, 41, 1), nc_shape=(1, 31, 41, 1), cc_shape=(1, 30, 40, 1))
